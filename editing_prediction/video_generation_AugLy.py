...
def main(args):
    ...
    if not Path(gt_match_csv).is_file():
        ...
        with open(gt_match_csv, 'a') as f_gt:
            ...
            writer_gt.writerow(["query_id", "ref_id", "query_start", "query_end", "ref_start", "ref_end"])
        with open(edit_query_meta_csv, 'a') as f_meta:
            writer_meta = csv.writer(f_meta, delimiter=',')
            writer_meta.writerow(["video_id", "duration_sec", "frames_per_sec", "width", "height", "rn", "base_query_id"])
    ...
    for i in range(args.sample_range[0], args.sample_range[1]):  
        ...
        if r_dura_org > 5:
            ...
            copy_ref = decord_clip_reader(path=ref_id, start=r_start, duration=r_dura_copy, count=count)
        else:
            continue
        ...
        query_all = decord_clip_reader(path=query_id, start=0.0, duration=q_dura_org, count=count)
        ...
        augly_method = [
            ...
            Pad(w_factor=pad_w_factor, h_factor=pad_w_factor, color=(pad_color_r,pad_color_g,pad_color_b)),
            ...
            OverlayDots(num_dots=overlay_num_dots, dot_type=overlay_dot_type, random_movement=overlay_random_movement),
            ...
            OverlayEmoji(emoji_path=emoji_path, x_factor=emoji_x_factor, y_factor=emoji_y_factor, opacity=emoji_opacity, emoji_size=emoji_size),
            
            BlendVideos(str(video_in2), opacity=opacity, overlay_size=overlay_size),
            HStack(str(video_in2)),
            VStack(str(video_in2)),
            Overlay(str(video_in2), x_factor=overlay_x_factor, y_factor=overlay_y_factor),
        ...
        out_meta_path = f"{args.output}/cvl_train_augly_metadata/Q4{str(i).zfill(5)}.json"
        with tempfile.TemporaryDirectory() as dir: 
            copy_vid_path = f"{dir}/copy_video.mp4"
            torchvision.io.write_video(filename=copy_vid_path, video_array=copy_video, fps=float(q_meta["frames_per_sec"]))        
        ...
        with open(gt_match_csv, 'a') as f_gt:
            ...
            q_end_sec = q_start_sec + copy_ref.shape[0]/float(q_meta["frames_per_sec"]) / factor
            row = [f"Q4{str(i).zfill(5)}", ref_id.stem, q_start_sec, q_end_sec, r_start, r_end]
            writer_gt.writerow(row)

        with open(edit_query_meta_csv, 'a') as f_meta:
            writer_meta = csv.writer(f_meta, delimiter=',')
            duration_sec = frameNo / float(q_meta["frames_per_sec"]) / factor
            row = [f"Q4{str(i).zfill(5)}", duration_sec, float(q_meta["frames_per_sec"]), int(q_meta["width"]), int(q_meta["height"]), i, query_id.stem]
...
if __name__=="__main__":
    ...
    parser.add_argument("--data_path", type=str, default="competition_data/train/reference/")
    parser.add_argument("--meta_path", type=str, default="competition_data/train/train_reference_metadata.csv")
