...
## train set含めた特徴抽出と評価の実行手順：

コンテナのビルド & コンテナにインタラクティブモードで入る
```
make interact-container
```

以下を実行すると、../full_outputsに結果がoutputされる。--dataset_paths に特徴抽出したい動画のディレクトリを指定する。
```
cd submission_src
conda run --no-capture-output -n condaenv python -m src.inference_full \
    ...
    --output_path ../full_outputs --gt_path /share-cvlab/yokoo/vsc/competition_data/train/train_matching_ground_truth.csv --tta
```

以下を実行すると、train setの評価結果が出力される。

```
make copy-full-results FULL_RESULTS_DIR=../full_outputs
make eval-train-subset  # runtime環境でのtrain setの評価（コンペのLBと同じく10%のデータ）
```

以下を実行すると、test setに対する推論とsubmissionの作成がされる。
