...
class VSCLitModule(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx):
        ...
        if self.val_transform is not None:
            if self.multi_crops and dataloader_idx == 0:
                all_crops = []
...
def train(args):
    ...
    aug_list = [
        ...
        RandomEmojiOverlay(p=0.1, emoji_size=(0.1, 0.3), x_pos=(0.0, 1.0), y_pos=(0.0, 1.0), seed=None),
        RandomNoise(p=0.2),
    ]
    aug_hard = [
        RandomRotation(p=0.1),
        RandomCompositeImageAndResizedCrop(
            train_paths, opacity_lower=0.5, size_lower=0.3, size_upper=0.7,
            input_size=input_size, moderate_scale_lower=0.7, hard_scale_lower=0.4, overlay_p=0.075,
            alpha_blending_p=0.075, horizontal_stacking_p=0.0, vertical_stacking_p=0.0, p=1.0,
        ),
        ShuffledAug(aug_list),
        convert2rgb,
        ToTensor(),
        RandomErasing(value='random', p=0.1),
        normalize_rgb,
    ]
...
if __name__ == "__main__":
    ...
    if args.mode == "predict":
        # predict(args)
        gt_matches = Match.read_csv(args.eval_gt_path, is_gt=True)
        aggregate_preds_and_evaluate(Path(args.pred_output_dir), gt_matches=None, only_train_query=False)
