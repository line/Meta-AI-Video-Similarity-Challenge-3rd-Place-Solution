...
class ISCGTDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        gt,
        query_dir,
        ...
        self.query_phase2_dir = query_phase2_dir
    ...
    def __getitem__(self, i):
        ...
        images = torch.stack(
            [
                ...
                self.transforms(ref_image),
                *[self.transforms(Image.open(self.noise_paths[j])) for j in js],
            ]
        )
        labels = torch.tensor([i, i] + [j + 1000000 for j in js])
...
class VSCLitModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        images = images.reshape(b * v, c, h, w).half()
        labels = labels.ravel()
...
def train(args):
    ...
    train_dataset = ISCGTDataset(
        gt=gt,
        query_dir=Path(args.data) / "query_images",
        query_phase2_dir=Path(args.data) / "query_images_phase2",
        ref_dir=Path(args.data) / "reference_images",
        noise_paths=noise_paths,
        transforms=Compose(aug_moderate),
        query_transforms=Compose(aug_moderate),
