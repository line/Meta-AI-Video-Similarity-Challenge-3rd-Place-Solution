...
class RandomCompositeImageAndResizedCrop(BaseTransform):
    ...
    def apply_transform(
        ...
        if composite_way == "overlay":
            ...
            image = Image.fromarray(np.concatenate([np.array(image), np.array(stacked_image)], axis=0))
            return RandomResizedCrop(self.input_size, scale=(self.moderate_scale_lower, 1.0))(image)
            ...
            image = Image.fromarray(np.concatenate([np.array(image), np.array(stacked_image)], axis=1))
            return RandomResizedCrop(self.input_size, scale=(self.moderate_scale_lower, 1.0))(image)
...
def train(args):
    ...
    # train_paths_cache_path = Path(args.data) / "train_paths_cache.pkl"
    # if train_paths_cache_path.exists():
    #     with open(train_paths_cache_path, "rb") as f:
    #         train_paths = pickle.load(f)
    # else:
    #     train_paths = (
    #         list((Path(args.data) / "train_images").glob('**/*.jpg'))
    #         + list((Path(args.data) / "reference_images").glob('**/*.jpg'))
    #     )
    #     with open(train_paths_cache_path, "wb") as f:
    ...
    aug_list = [
        ...
        RandomEmojiOverlay(p=0.05, emoji_size=(0.1, 0.3), x_pos=(0.0, 1.0), y_pos=(0.0, 1.0), seed=None),
        RandomNoise(p=0.1),
    ]
    aug_hard = [
        RandomRotation(p=0.05),
        RandomCompositeImageAndResizedCrop(
            train_paths, opacity_lower=0.6, size_lower=0.3, size_upper=0.7,
            input_size=input_size, moderate_scale_lower=0.8, hard_scale_lower=0.5, overlay_p=0.05,
            alpha_blending_p=0.05, horizontal_stacking_p=0.0, vertical_stacking_p=0.0, p=1.0,
        ),
...
def aggregate_preds_and_evaluate(
    ...
    if args.multi_crops:
        sorted_query_all_frame_pos = np.concatenate([sorted_query_all_frame_pos for _ in range(num_views)], axis=1)
        sorted_ref_all_frame_pos = np.concatenate([sorted_ref_all_frame_pos for _ in range(num_views)], axis=1)
        sorted_noise_all_frame_pos = np.concatenate([sorted_noise_all_frame_pos for _ in range(num_views)], axis=1)

    video_features = {
        "query": _construct_video_features(
            video_ids=query_ids, embeddings=sorted_query_all_preds, timestamps=sorted_query_all_frame_pos
        ),
        ...
        "noise": _construct_video_features(
            video_ids=noise_ids, embeddings=sorted_noise_all_preds, timestamps=sorted_noise_all_frame_pos
        ),
    }
...
if __name__ == "__main__":
    """Usage:  
    cv11
    CUDA_VISIBLE_DEVICES=4,5,6,7 python v41.py \
        -a timm/vit_base_patch16_clip_224.openai_ft_in12k --seed 22999 --workers 8 \
        --epochs 10 --lr 0.001 --wd 1e-6 --optimizer sgd --batch-size 32 --val-batch-size 16 \
        --pos-margin 0.0 --neg-margin 1.0 --sample_size 100000 \
        --input-size 224 --memory-size 5000 --feature_dim 512 \
    ...
    CUDA_VISIBLE_DEVICES=4,5,6,7 python v41.py \
        -a timm/vit_base_patch16_clip_224.openai_ft_in12k --seed 22999 --workers 8 \
        --epochs 15 --lr 0.001 --wd 1e-6 --optimizer sgd --batch-size 32 --val-batch-size 16 \
        --pos-margin 0.0 --neg-margin 1.0 --sample_size 100000 \
        --input-size 224 --memory-size 5000 --feature_dim 512 \
        --ddp_strategy deepspeed_stage_2 --warmup_steps 0 \
        --query_video_dir /app/data/vsc/competition_data/train/query \
    ...
    CUDA_VISIBLE_DEVICES=4,5,6,7 python v41.py \
        ...
        --epochs 3 --lr 0.001 --wd 1e-6 --optimizer sgd --batch-size 32 --val-batch-size 16 \
        --pos-margin 0.0 --neg-margin 1.0 \
        --input-size 224 --memory-size 20000 --feature_dim 512 --sample_size 500000 \
        --ddp_strategy deepspeed_stage_2 --warmup_steps 0 \
        --query_video_dir /app/data/vsc/competition_data/train/query \
        --ref_video_dir /app/data/vsc/competition_data/train/reference \
        --noise_video_dir /app/data/vsc/competition_data/test/reference \
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 python v41.py \
        ...
        --epochs 10 --lr 0.001 --wd 1e-6 --optimizer sgd --batch-size 32 --val-batch-size 16 \
        --pos-margin 0.0 --neg-margin 1.0 \
        --input-size 224 --memory-size 20000 --feature_dim 512 --sample_size 100000 \
        --ddp_strategy deepspeed_stage_2 --warmup_steps 0 \
        --query_video_dir /app/data/vsc/competition_data/train/query \
        --ref_video_dir /app/data/vsc/competition_data/train/reference \
        --noise_video_dir /app/data/vsc/competition_data/test/reference \
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 python v41.py \
        ...
        --epochs 10 --lr 0.001 --wd 1e-6 --optimizer sgd --batch-size 32 --val-batch-size 16 \
        --pos-margin 0.0 --neg-margin 1.0 \
        --input-size 224 --memory-size 5000 --feature_dim 512 --sample_size 100000 \
        --ddp_strategy deepspeed_stage_2 --warmup_steps 0 \
        --query_video_dir /app/data/vsc/competition_data/train/query \
        --ref_video_dir /app/data/vsc/competition_data/train/reference \
        --noise_video_dir /app/data/vsc/competition_data/test/reference \
